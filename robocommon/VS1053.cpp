...
static uint8_t PrintStatus(const CLS1_StdIOType *io) {
  ...
  if (VS_ReadRegister(VS_DECODE_TIME, &val)==ERR_OK) {
    UTIL1_strcpy(buf, sizeof(buf), (unsigned char*)"0x");
    UTIL1_strcatNum16Hex(buf, sizeof(buf), val);
    UTIL1_strcat(buf, sizeof(buf), (unsigned char*)" seconds\r\n");
  } else {
    UTIL1_strcpy(buf, sizeof(buf), (unsigned char*)"ERROR\r\n");
  }
  ...
  if (VS_ReadRegister(VS_AUDATA, &val)==ERR_OK) {
    ...
    UTIL1_strcat(buf, sizeof(buf), (val&1)?(unsigned char*)"stereo\r\n":(unsigned char*)"mono\r\n");
  } else {
    UTIL1_strcpy(buf, sizeof(buf),(unsigned char*) "ERROR\r\n");
  }
  ...
  if (VS_ReadRegister(VS_VOL, &val)==ERR_OK) {
    UTIL1_strcpy(buf, sizeof(buf), (unsigned char*)"0x");
    UTIL1_strcatNum16Hex(buf, sizeof(buf), val);
    UTIL1_strcat(buf, sizeof(buf), (unsigned char*)" (0x0000 full, 0xFEFE silence)\r\n");
  } else {
    UTIL1_strcpy(buf, sizeof(buf), (unsigned char*)"ERROR\r\n");
  }
...
uint8_t VS_ParseCommand(const unsigned char *cmd, bool *handled, const CLS1_StdIOType *io)
{
  ...
  if (UTIL1_strcmp((char*)cmd, CLS1_CMD_HELP)==0 || UTIL1_strcmp((char*)cmd, "VS1053 help")==0) {
    ...
    *handled = TRUE;
    ...
    if (UTIL1_xatoi(&p, (int32_t *)&val32u)==ERR_OK) {
      return VS_SetVolume((uint16_t)val32u);
    } else {
      CLS1_SendStr((unsigned char*)"Failed reading volume", io->stdErr);
      return ERR_FAILED;
    }
