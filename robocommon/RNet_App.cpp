...
static uint8_t HandleDataRxMessage(RAPP_MSG_Type type, uint8_t size, uint8_t *data, RNWK_ShortAddrType srcAddr, bool *handled, RPHY_PacketDesc *packet) {
  ...
  switch(type) {
    case RAPP_MSG_TYPE_DATA: /* generic data message */
      ...
      *getConsole().getUnderlyingIoStream() << "Data: ";
      //CLS1_SendNum8u(val, io->stdOut);
      *getConsole().getUnderlyingIoStream() << val;
      //CLS1_SendStr((unsigned char*)" from addr 0x", io->stdOut);
      *getConsole().getUnderlyingIoStream() << " from addr 0x";
      buf[0] = '\0';
      ...
      *getConsole().getUnderlyingIoStream() << buf;
#endif /* PL_HAS_SHELL */      
      return ERR_OK;
...
static void Init(void) {
	RNWK_ShortAddrType adr;
#if PL_HAS_REMOTE
#if PL_IS_ROBO
	adr = REMOTE_ADDR_ROBO; //Robo
#else
	adr = REMOTE_ADDR_CONTROL; //RemoteControl
#endif
#else
	adr = RNWK_ADDR_BROADCAST;
#endif
	if (RAPP_SetThisNodeAddr(adr)!=ERR_OK) { /* set a default address */
